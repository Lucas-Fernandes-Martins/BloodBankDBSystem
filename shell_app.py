...
def get_input(prompt, default=None):
    """Ajuda a obter entrada com um valor padrao."""
    if default:
...
def load_queries(filepath):
    """Analisa o arquivo SQL e retorna um dicionario de {nome: sql}."""
    queries = {}
    current_name = None
    current_sql = []
...
def execute_query(query_name, sql, params=None, fetch_results=True):
    """Executa uma consulta SQL e retorna resultados ou imprime eles."""
    try:
...
def login(queries):
    ...
    if results:
        return results[0] # Retorna dict com 'role' e 'pessoaid'
    else:
        ...
        while True:
            ...
            if role == 'admin' or role == 'instituicao':
                options = [
                    ...
                    ('(*) Doações por Mês (Centro de Coleta)', run_donations_per_month)
                ]
            elif role == 'medico':
                options = [
                    ('Listar Doadores', run_simple_query('list_doadores')),
